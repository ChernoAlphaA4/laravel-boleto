...
//      $this->Ln(4);
  ...
  protected function Topo($i)
  {
    ...
    $this->textFitCell(75, $this->cell, $this->_($this->boleto[$i]->getBeneficiario()->getNomeDocumento()), 'LR', 0, 'L');
  ...
  protected function Bottom($i)
  {
    ...
    foreach ($infos as $array) {
      if (is_array($array)){
        $array = collect($array)->each(function (&$item){
          $item->person_name = $item->person_name . (!is_null($item->person_enrol_number) && isset($item->person_enrol_number) ? ' (' . $item->person_enrol_number . ')' : '');
        });
    ...
    $this->Cell(0, $this->cell, $this->_($beneficiary), 'LR', 1);
  ...
  private function listaLinhas($lista, $pulaLinha)
  {
    foreach ($lista as $d) {
      if (is_array($d)) {
        // Header
        $this->Cell(70, 4, 'Pessoa', 1);
        $this->Cell(60, 4, $this->_('Serviço'), 1);
        $this->Cell(20, 4, 'Parcela', 1);
        $this->Cell(20, 4, 'Valor', 1);
        $this->Ln();
        // Data
        foreach ($d as $row) {
          $this->Cell(70, 4, $this->_($row->person_name . (!is_null($row->person_enrol_number) && isset($row->person_enrol_number) ? ' ('.$row->person_enrol_number.')' : '')), 1);
